...
hstr += '\n'
hstr += timestr + '\n'
hstr += 'x-amz-copy-source:my_bucketes/ac.conf\n'
hstr += '/my_1bucket/a.conf'
print('hstr:%s' % (hstr,))
...
print aws
header = {"Host":"192.168.205.43:80","Date":timestr,"Authorization":aws,'x-amz-copy-source':"my_bucketes/ac.conf"}
#header = {"Date":timestr,"Authorization":aws,"x-amz-acl": "public-read-write"}
req = requests.put(url = 'http://192.168.205.43:80/my_1bucket/a.conf',
                   headers = header)
