...
class TesRgwApi(unittest.TestCase):
    ...
    def test_42_link_bucket(self):
        content =  bucket("bucket=s3_object_bucket_4&stats=True","get",True)
        if content[0] == 200:
            ids = content[1]["id"]
            self.assertEqual(bucket("uid=testid&bucket=s3_object_bucket_4&bucket-id=%s"%ids,"put"),200)
        else:
            self.assertEqual(404,200)
